...
document.querySelector('#test').innerText = "Learning DOM"
...
list.forEach((elm) => elm.innerText = "allTopics");

// 13. Select all the elements with class `list` and store in variable `listOfSelectedTopics`
let listOfSelectedTopics = document.querySelectorAll('.list');

// 14. Select the first li element inside the `ul` element using `>` (direct child) and store in `firstLi`
let firstLi = document.querySelectorAll('ul > li:nth-child(1)');
...
let lastElement = document.querySelector('.topics').lastElementChild;
console.log(typeof(lastElement)) /Object

// 26. Select the element with type `fieldset` and store in a variable named `fieldsetElm`.
let fieldsetElm = document.querySelector('fieldset');

// 27. Select the parent element of the element stored in `fieldsetElm` variable (#5) and log the `typeof` the element.
let fieldsetParent = document.querySelector('fieldset').parentElement;
console.log(typeof(fieldsetParent));